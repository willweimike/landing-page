...
const I18N = {
  zh: {
    ...
    feat: {
      ...
      f5t: '程序技能庫（10 內建）',
      f5d: '<code>ProcedureSkillLibrary</code> 內建 10 個技能（api-debugging、browser-navigation、code-editing、codebase-documentation、process-management、systematic-debugging、technical-spike、TDD、web-research、website-inspection），每步動態選取最相關技能載入。',
      f6t: '後執行學習迴路',
      ...
      f9t: '可擴展工具集目錄',
      f9d: '<code>TOOLSET_CATALOG</code> 涵蓋 6 個已實作工具集：file（5 工具）、terminal（6 工具）、browser（11 工具）、web（2 工具）、search、skills，以及 memory、delegate、cronjob、computer_use 等規劃中工具集。預設啟用：file、terminal、search、skills。',
      f10t: '完整事件溯源稽核',
    ...
    arch: {
      ...
      techtitle: 'TOOLSET_CATALOG — 實際工具集',
    },
    toolsets: {
      tag: '內建技能',
      h2: '10 個 Bundled Procedure Skills',
      desc: '每個技能以 Markdown 格式定義，包含名稱、描述、所需工具與平台',
      note: '每次執行 Turn 時，<code>model.select_procedure_skills()</code> 依可用工具名稱動態篩選並載入最相關技能；執行成功且達到門檻後，<code>LearningLoop</code> 將自動起草新技能並生成 <code>LearningProposal</code>（CANDIDATE 狀態）。',
    },
    demo: {
      tag: '真實事件序列',
      h2: 'AgentLoop 執行日誌',
  ...
  en: {
    ...
    hero: {
      badge: 'Python · Running Autonomously',
      ...
      stat2: 'Implemented Toolsets',
      stat3: 'Scoring Dimensions',
      stat4: 'Beam Width',
    },
    about: {
    ...
    feat: {
      ...
      f5t: 'Procedure Skill Library (10 Built-in)',
      f5d: '<code>ProcedureSkillLibrary</code> ships with 10 built-in skills (api-debugging, browser-navigation, code-editing, codebase-documentation, process-management, systematic-debugging, technical-spike, TDD, web-research, website-inspection). The most relevant skills are dynamically loaded each step.',
      ...
      f9d: '<code>TOOLSET_CATALOG</code> covers 6 implemented toolsets: file (5 tools), terminal (6 tools), browser (11 tools), web (2 tools), search, skills, plus planned: memory, delegate, cronjob, computer_use. Default enabled: file, terminal, search, skills.',
    ...
    arch: {
      ...
      techtitle: 'TOOLSET_CATALOG — Implemented Toolsets',
    },
    toolsets: {
      tag: 'Built-in Skills',
      h2: '10 Bundled Procedure Skills',
      desc: 'Each skill is defined in Markdown format with name, description, required tools, and platform',
      note: 'On each Turn, <code>model.select_procedure_skills()</code> dynamically filters and loads the most relevant skills based on available tool names. After a successful Run meeting the threshold, <code>LearningLoop</code> auto-drafts a new skill and generates a <code>LearningProposal</code> (CANDIDATE status).',
    },
    demo: {
